#!/usr/bin/env node
/**
 * Record a single Thinkspace Projects flow video (headed, slow, always on video).
...
const flowPath = resolve(rootDir, 'src/data/thinkspace/project-flow-demo.json');
let flowSteps = [];
...
console.log('\n--- Thinkspace Projects — demo flow video (26 steps) ---\n');
...
if (flowSteps.length) {
  console.log('Steps (see docs/THINKSPACE_PROJECTS_WORKFLOW.md):\n');
...
const demo = spawnSync(
  ...
  [
    'playwright',
    'test',
    'tests/thinkspace/demo/project-module-flow.ui.spec.ts',
...
if (demo.status === 0) {
  console.log('\n✅ Projects demo flow completed.');
  console.log('   Open test-results/ and find video.webm for the walkthrough test.\n');
} else {
  console.log('\n❌ Projects demo flow failed — check test-results/ for video/trace anyway.\n');
}
