#!/usr/bin/env node
/**
 * Record a single Thinkspace Travel flow video (headed, slow, always on video).
...
const flowPath = resolve(rootDir, 'src/data/thinkspace/travel-flow-demo.json');
let flowSteps = [];
...
console.log('\n--- Thinkspace Travel — demo flow video ---\n');
...
if (flowSteps.length) {
  console.log('Steps (see docs/THINKSPACE_TRAVEL_WORKFLOW.md):\n');
...
const demo = spawnSync(
  ...
  [
    'playwright',
    'test',
    'tests/thinkspace/demo/travel-module-flow.ui.spec.ts',
...
if (demo.status === 0) {
  console.log('\n✅ Travel demo flow completed.');
  console.log('   Open test-results/ and find video.webm for the walkthrough test.\n');
} else {
  console.log('\n❌ Travel demo flow failed — check test-results/ for video/trace anyway.\n');
}
